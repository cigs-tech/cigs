...
describe('Classification using ChainSmoker', () => {
  ...
  const classificationSmoker = cig("SentimentAnalysis", inputSchema, (config) => {
    config.setDescription('Classify the sentiment of the given text');
    config.setModel('gpt-4o-2024-08-06');
  }).classify(labels, (config) => {
    config.addInstruction('Determine if the sentiment is positive, negative, or neutral.');
  });

  it('should classify positive sentiment correctly', async () => {
    const result = await classificationSmoker.run({ text: 'I love this product!' });
    expect(labels).toContain(result);
  }, 30000);

  it('should classify negative sentiment correctly', async () => {
    const result = await classificationSmoker.run({ text: 'This is terrible.' });
    expect(labels).toContain(result);
  }, 30000);

  it('should handle string input directly', async () => {
    const result = await classificationSmoker.run('This is a neutral statement.');
    expect(labels).toContain(result);
  }, 30000);
  ...
  it('should handle classification with only one label', async () => {
    const singleLabelSmoker = cig("SingleLabelClassification", inputSchema, (config) => {
  ...
  it('should handle multiple consecutive classifications', async () => {
    ...
    const results = await Promise.all(texts.map(text => classificationSmoker.run({ text })));
  ...
  it('should handle mixed sentiment', async () => {
    const result = await classificationSmoker.run({ text: 'The product is great but the service is poor.' });
    expect(labels).toContain(result);
  }, 30000);

  it('should handle sarcasm', async () => {
    const result = await classificationSmoker.run({ text: 'Oh great, another delay. Just what I needed.' });
    expect(labels).toContain(result);
  }, 30000);
});
